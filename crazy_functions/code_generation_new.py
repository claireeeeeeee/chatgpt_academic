...
@CatchException
def code_generation_new(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    history = []    # 清空历史，以免输入溢出
...
"""
            ...
            raise RuntimeError(response.content.decode())
        save_history(task_id, 'Coder', code_say)
...
"""
            ...
            raise RuntimeError(response.content.decode())
        save_history(task_id, 'Test', test_say)
