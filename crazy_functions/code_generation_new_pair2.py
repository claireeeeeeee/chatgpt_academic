...
@CatchException
def code_generation_new_pair2(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    history = []    # 清空历史，以免输入溢出
...
"""
        ...
        try:
            i_say = f"""
Here is the requirement: ```{prompt}``` and the report from the developer: ```{code_say}```
...
def save_to_jsonl(task_id, code_segments):
    ...
    with open("/home/cli776/human-eval/new_pair/samples3.jsonl", "a") as file:
        file.write(json.dumps(data) + "\n")

def save_history(key1, key2, plan_say):
    filename="/home/cli776/human-eval/new_pair/recoding3.jsonl"
